...
def get_timeseries_summary_per_day(timeseries, timeseries_elements, metadata, summary="median"):
    ...
    # only do so as we are summarising the observations with median/IQR
    ...
    for k in time_batches.keys():
        ...
        if summary == "median":
            # median
            summary_ts[(timestamp.astype('datetime64[D]'))] = np.percentile(ts, q=50, interpolation='lower')
        elif summary == "IQR":
        ...
        median_pct = ngrams[np.argwhere(ts == np.percentile(ts, q=50, interpolation='lower'))][0, 0].split("<br>")[-1]
        lower_pct = ngrams[np.argwhere(ts == np.percentile(ts, q=25, interpolation='lower'))][0, 0].split("<br>")[-1]
        upper_pct = ngrams[np.argwhere(ts == np.percentile(ts, q=75, interpolation='lower'))][0, 0].split("<br>")[-1]
        ...
        meta[(timestamp.astype('datetime64[D]'))] = {"annotation": m, "number of ngrams":
            time_batches[k]["number of ngrams"], "25%": lower_pct, "50%": median_pct, "75%": upper_pct}
...
def get_timestamp_gaps_dates(dates, output_file="/tmp/gap_days.html"):
    ...
    months_days = [(int(i.split('-')[1]), int(i.split('-')[2]), int(i.split('-')[0])) for i in dates]
    gap_days = []
    gap_index = []
    for i in range(len(months_days[1:])):
        day1 = months_days[i]
        day2 = months_days[i + 1]
        # if in same month, diff the days
        if day2[0] == day1[0]:
