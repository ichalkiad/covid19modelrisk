...
mu[1] <- y[1];  ## not tested
  ...
  for (n in 2:N) {
      ...
      mu[n] <- mu[n-1]*exp(u+b1*mu[n-1]+b4*(mu[n-1]^2)+e[n]); 
...
lambda~uniform(-1,1);
u ~ normal(0.1,0.001);
b1 ~ normal(-0.000000054,0.0000000001);
b4 ~ normal(-0.000000000001,0.00000000000000001);
sig2 ~  gamma(1,1); 
e~normal(0,sig2);
...
#### initial values
inits =list(list(lambda=0,u=0.1,b1=-0.000000054, b4=-0.000000000001,sig2=0.01,e=rep(0.01,N),sigob=1000))  ### 134
...
Epars_m <- apply( as.data.frame(fit@sim[[1]][[1]])[4001:40000,(7+N):(6+2*N)] , 2 , mean )

Epars_5 <- apply( as.data.frame(fit@sim[[1]][[1]])[4001:40000,(7+N):(6+2*N)] , 2 , quantile, probs=0.05 )

Epars_9 <- apply( as.data.frame(fit@sim[[1]][[1]])[4001:40000,(7+N):(6+2*N)] , 2 , quantile, probs=0.95 )
...
traceplot(fit, pars=c("u","b1","b4"), inc_warmup = T,  nrow = 3, ncol = 1, window = NULL, include = TRUE)
...
plot(y[1:m],ylim=c(min(y[1:m]),max(y[1:m],Epars_9[1:m])),ylab="Count",xlab="Time",main="In-sample fit results of M4 model for Germany",pch=19)
par(new=TRUE)
...
plot(y[1:m],ylim=c(min(y[1:m]),max(y[1:m],Epars_9[1:m])),ylab="Count",xlab="Time",main="In-sample fit results of M4 model for Germany",pch=19)
par(new=TRUE)
plot.ts(Epars_m[1:m],col="red",ylim=c(min(y[1:m]),max(y[1:m],Epars_9[1:m])),ylab="Count",xlab="Time",main="In-sample fit results of M4 model for Germany",lwd = 2)
...
x[i+1]=x[i]*exp(u+b1*x[i]+b4*(x[i]^2))
}
x=round(x)
x
